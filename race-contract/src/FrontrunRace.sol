...
// ERC721 (0xf8c20abd9c233a9de999c2ac6f1628df3f64efb0), no validator swap, no locks.
//
...
contract FrontrunRace {
    ...
    uint256 public kingGround;                  // king's ground at coronation — event/lore only (crown compares live groundOf)
    ...
    function burnInto(uint256 sacrifice, uint256 survivor) external {
        ...
        // ---- effects: ALL state settled before any external call (full CEI) ----
        dead[sacrifice] = true;
        ...
        // crown: empty until the first burn. To take it you must beat the king's
        // CURRENT ground — not a frozen snapshot. Because every living piece accrues
        // base ground at the same rate, comparing live grounds cancels the base and
        // reduces to absorbed-vs-absorbed: you overtake by out-absorbing, never by
        // merely waiting. If the old king was itself just sacrificed, groundOf(king)
        // is 0 and the survivor inherits by absorption.
        bool take;
        if (!crownClaimed) take = true;
        else if (king == sacrifice) take = true;
        else take = survivorGround > groundOf(king);
        if (take) {
        ...
        // ---- interaction LAST: the fire. No state is touched after this call. ----
